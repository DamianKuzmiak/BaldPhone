    ...
    public static String resolveLookupKey(
        ...
        if (!TextUtils.isEmpty(cachedLookupUri)) {
            try {
                ...
                // It's better to catch specific exceptions like MalformedURLException
        ...
        if (!TextUtils.isEmpty(number)) {
            ...
            if (!TextUtils.isEmpty(normalized)) {
                ...
                // For PhoneLookup, the URI resolves directly to the contact.
    ...
    private static String queryLookupKey(
        ...
        try (Cursor c = resolver.query(contactUri, projection, null, null, null)) {
            if (c != null && c.moveToFirst()) {
                // Using getColumnIndex instead of getColumnIndexOrThrow is safer.
            ...
            // Log exceptions that might occur during the query.
    ...
    public static int getRawContactId(int contactId, @NonNull ContentResolver resolver) {
        try (final Cursor c =
            ...
            if (c != null && c.moveToNext()) {
                int columnIndex = c.getColumnIndex(ContactsContract.RawContacts._ID);
                if (columnIndex != -1) {
                    return c.getInt(columnIndex);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Failed to get raw contact ID for contact: " + contactId, e);
        }
        return -1;
    }
}
